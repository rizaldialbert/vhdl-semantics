...
next
  case False
  then obtain \<tau>' where cyc: "maxtime, \<sigma>, \<gamma>, \<theta>, def \<turnstile> <nand3, \<tau>> \<longrightarrow>\<^sub>c \<tau>'" and
    big: "maxtime, next_time maxtime \<tau>',
                   next_state maxtime \<tau>' \<sigma>,
                   next_event maxtime \<tau>' \<sigma>,
                   add_to_beh \<sigma> \<theta> maxtime (next_time maxtime \<tau>'), def \<turnstile>
  ...
  have 0: "\<And>n. n < maxtime \<Longrightarrow> \<tau>' n = 0"
    using b_conc_exec_preserve_trans_removal[OF cyc assms(2)] by auto
  ...
  hence big2: "maxtime, next_time maxtime \<tau>', \<sigma>2, \<gamma>2,
                        add_to_beh \<sigma> \<theta> maxtime (next_time maxtime \<tau>'), def
  ...
  { assume "\<tau>' \<noteq> 0"
    ...
    { assume "\<not> disjnt \<gamma> {A, B}"
      ...
      hence \<tau>'_def: "\<tau>' = trans_post_raw C x (\<sigma> C) \<tau> maxtime 1"
        using cyc unfolding nand3_def
        by (meson \<open>\<not> disjnt \<gamma> {A, B}\<close> b_seq_exec_deterministic conc_cases(1) disjnt_sym)
      hence "\<tau>' maxtime = 0"
...
proof (induction rule:b_simulate_fin.induct)
  ...
  have IH: " next_time t \<tau>' \<le> i \<Longrightarrow>
             bval_of (signal_of (def C) (get_beh res) C (Suc i)) \<longleftrightarrow>
    ...
    using 1(5)[OF `cs = nand3` `maxtime = Suc i` h0 h1 h2 _ h3 h4 `conc_wt \<Gamma> cs` `styping \<Gamma> (next_state t \<tau>' \<sigma>)`
  ...
  { assume "i < next_time t \<tau>'"
    ...
    { assume "A \<notin> \<gamma> \<and> B \<notin> \<gamma>"
      ...
      { assume "maxtime < next_time t \<tau>'"
        hence pre: "\<And>n. n \<le> next_time t \<tau>' \<Longrightarrow>  (add_to_beh \<sigma> \<theta> t (next_time t \<tau>')) n =  get_beh res n"
          using borderline_big_step[OF next_big_step `t , \<sigma> , \<gamma> , \<theta>, def \<turnstile> <cs , \<tau>> \<longrightarrow>\<^sub>c \<tau>'` `t \<le> maxtime` `maxtime < next_time t \<tau>'` 1(10)]
          by auto
    ...
    { assume "A \<in> \<gamma> \<or> B \<in> \<gamma>"
      ...
      { assume "bval_of (\<sigma> C) \<longleftrightarrow> \<not> (bval_of (\<sigma> A) \<and> bval_of (\<sigma> B))"
        ...
        { assume "next_time t \<tau>' \<le> maxtime"
          ...
          hence ?case
            ...
            `bval_of (\<sigma> C) \<longleftrightarrow> \<not> (bval_of (\<sigma> A) \<and> bval_of (\<sigma> B))` by auto }
        moreover
        { assume "maxtime < next_time t \<tau>'"
          hence pre: "\<And>n. n \<le> next_time t \<tau>' \<Longrightarrow>  (add_to_beh \<sigma> \<theta> t (next_time t \<tau>')) n =  get_beh res n"
            using borderline_big_step[OF next_big_step `t , \<sigma> , \<gamma> , \<theta>, def \<turnstile> <cs , \<tau>> \<longrightarrow>\<^sub>c \<tau>'` `t \<le> maxtime` `maxtime < next_time t \<tau>'` 1(10)]
      ...
      { assume "(\<not> bval_of (\<sigma> C) \<longleftrightarrow> \<not> (bval_of (\<sigma> A) \<and> bval_of (\<sigma> B)))"
        ...
        hence "signal_of (def C) (get_beh res) C (Suc i) = next_state t \<tau>' \<sigma> C"
        ...
        finally have "signal_of (def C) (get_beh res) C (Suc i) = Bv (\<not> (bval_of (\<sigma> A) \<and> bval_of (\<sigma> B)))"
          by auto
        hence ?case
          using sigA sigB by auto }
      ultimately have ?case by auto }
  ...
  { assume "next_time t \<tau>' \<le> i"
    with IH have IH': "bval_of (signal_of (def C) (get_beh res) C (Suc i)) \<longleftrightarrow>
      (\<not> (bval_of (signal_of (next_state t \<tau>' \<sigma> A) (\<tau>'(next_time t \<tau>' := 0)) A i) \<and>
          bval_of (signal_of (next_state t \<tau>' \<sigma> B) (\<tau>'(next_time t \<tau>' := 0)) B i)))"
      by auto
...
next
  case (3 t maxtime \<sigma> \<gamma> def cs \<tau>)
  then show ?case by auto
qed
...
theorem nand3_correctness:
  ...
  shows "bval_of (signal_of (def C) (get_beh res) C (Suc i)) \<longleftrightarrow>
...
proof (cases " \<tau> 0 = 0")
  ...
  { assume "Bv (\<not> (bval_of (def A) \<and> bval_of (def B))) = def C"
    ...
    hence * : "next_time 0 \<tau> \<le> i \<Longrightarrow>
               bval_of (signal_of (def C) (get_beh res) C (Suc i)) =
    ...
    next
      ...
      ultimately show ?thesis
        by auto
    qed
    ultimately have IR: "next_time 0 \<tau> \<le> i \<Longrightarrow> bval_of (signal_of (def C) (get_beh res) C (Suc i)) =
           (\<not> (bval_of (signal_of (def A) \<tau> A i) \<and>  bval_of (signal_of (def B) \<tau> B i)))"
      by auto
    { assume "i < next_time 0 \<tau>"
      ...
      have "\<And>n. n < next_time 0 \<tau> \<Longrightarrow> get_beh res n = ((add_to_beh def 0 0 (next_time 0 \<tau>))(next_time 0 \<tau> := Some \<circ> next_state 0 \<tau> def)) n"
        using beh_res2[OF bigstep s1 _ _ s3]
        by (metis bigstep case_timesup(1) fun_upd_apply nat_less_le)
      ...
      { assume "Suc i < next_time 0 \<tau>"
        have "inf_time (to_trans_raw_sig (get_beh res)) C (Suc i) = Some 0"
          by (metis (no_types, hide_lams) \<open>0 < next_time 0 \<tau>\<close> \<open>Suc i < next_time 0 \<tau>\<close> add_to_beh_def
          bigstep case_timesup(1) inf_time_update le0 le_less le_less_trans less_irrefl
          less_not_refl order_refl zero_fun_def)
        hence "signal_of (def C) (get_beh res) C (Suc i) = def C"
          by (smt \<open>0 < next_time 0 \<tau>\<close> \<open>Suc i < next_time 0 \<tau>\<close> add_to_beh_def bigstep case_timesup(1)
          comp_apply fun_upd_same leD some_inf_time' to_signal_def trans_some_signal_of)
        hence "bval_of (signal_of (def C) (get_beh res) C (Suc i)) =
           (\<not> (bval_of (signal_of (def A) \<tau> A i) \<and>  bval_of (signal_of (def B) \<tau> B i)))"
          by (metis \<open>Bv (\<not> (bval_of (def A) \<and> bval_of (def B))) = def C\<close> \<open>signal_of (def A) \<tau> A i =
          def A\<close> \<open>signal_of (def B) \<tau> B i = def B\<close> val.sel(1)) }
      moreover
      { assume "Suc i = next_time 0 \<tau>"
        ...
        have "get_beh res = (add_to_beh def 0 0 (next_time 0 \<tau>))(Suc i := Some \<circ> next_state 0 \<tau> def)"
          using maxtime_maxtime_bigstep[OF bigstep' s1 s3] unfolding `Suc i = next_time 0 \<tau>`
          by auto
        have "inf_time (to_trans_raw_sig (get_beh res)) C (Suc i) = Some (Suc i)"
          by (metis \<open>get_beh res = (add_to_beh def 0 0 (next_time 0 \<tau>)) (Suc i := Some \<circ> next_state
          0 \<tau> def)\<close> fun_upd_same some_inf_time)
        hence "signal_of (def C) (get_beh res) C (Suc i) = next_state 0 \<tau> def C"
          by (metis \<open>get_beh res = (add_to_beh def 0 0 (next_time 0 \<tau>)) (Suc i := Some \<circ> next_state
          0 \<tau> def)\<close> fun_upd_same trans_some_signal_of)
        also have "... = def C"
          using assms(2)  by (metis (mono_tags, hide_lams) domIff next_state_def override_on_def
          to_trans_raw_sig_def zero_fun_def zero_option_def)
        finally have "signal_of (def C) (get_beh res) C (Suc i) = def C"
          by auto
        hence "bval_of (signal_of (def C) (get_beh res) C (Suc i)) =
           (\<not> (bval_of (signal_of (def A) \<tau> A i) \<and>  bval_of (signal_of (def B) \<tau> B i)))"
          by (metis \<open>Bv (\<not> (bval_of (def A) \<and> bval_of (def B))) = def C\<close> \<open>signal_of (def A) \<tau> A i =
          def A\<close> \<open>signal_of (def B) \<tau> B i = def B\<close> val.sel(1)) }
      ultimately have "bval_of (signal_of (def C) (get_beh res) C (Suc i)) =
           (\<not> (bval_of (signal_of (def A) \<tau> A i) \<and>  bval_of (signal_of (def B) \<tau> B i)))"
        by auto }
    hence ?thesis
  ...
  { assume "Bv (\<not> (bval_of (def A) \<and> bval_of (def B))) \<noteq> def C"
    ...
    ultimately have IR: "1 \<le> i \<Longrightarrow> bval_of (signal_of (def C) (get_beh res) C (Suc i)) \<longleftrightarrow>
                                              \<not> (bval_of (signal_of (def A) ?\<tau>' A i) \<and> bval_of (signal_of (def B) ?\<tau>' B i))"
      by auto
    have " ?\<tau>' 0 = 0"
    ...
    have " get_beh res 1 = ((add_to_beh def 0 0 (next_time 0 ?\<tau>')) (next_time 0 ?\<tau>' := Some \<circ> next_state 0 ?\<tau>' def)) 1"
      using beh_res2[OF bigstep ind1' `next_time 0 ?\<tau>' \<le> Suc i` _ ind3 `1 \<le> next_time 0 ?\<tau>'`]
      by auto
    ...
    hence ?thesis
      ...
      by (metis One_nat_def \<open>next_time 0 (trans_post_raw C (Bv (\<not> (bval_of (def A) \<and> bval_of (def
      B)))) (def C) \<tau> 0 1) \<le> Suc i\<close> le_Suc_eq nat.inject ntime) }
  ultimately show ?thesis
    by auto
next
  case False
  ...
  { assume "Bv (\<not> (bval_of (def A) \<and> bval_of (def B))) \<noteq> def C"
    ...
    moreover have "0 \<le> Suc i"
      by auto
    obtain \<tau>'' where cyc: "0 , \<sigma>' , \<gamma>' , 0, def \<turnstile> <nand3 , (?\<tau>' (t':=0))> \<longrightarrow>\<^sub>c \<tau>''" and
      bigstep'': "(Suc i), next_time 0 \<tau>'' , next_state 0 \<tau>'' \<sigma>' , next_event 0 \<tau>'' \<sigma>' , add_to_beh \<sigma>' 0 0 (next_time 0 \<tau>''), def \<turnstile>
      ...
      { assume "\<not> bval_of (\<sigma>' C) \<longleftrightarrow> \<not> (bval_of (\<sigma>' A) \<and> bval_of (\<sigma>' B))"
       ...
       (\<not> (bval_of (signal_of (\<sigma>'' A) (\<tau>''(next_time 0 \<tau>'' := 0)) A i) \<and> bval_of (signal_of (\<sigma>'' B) (\<tau>''(next_time 0 \<tau>'' := 0)) B i)))"
        ...
        also have "... = Some o \<sigma>''"
          ...
          by auto
        hence "signal_of (def C) (get_beh res) C 1 = \<sigma>'' C"
          by (meson trans_some_signal_of)
        also have "... = (let m =  \<tau>'' 1 in override_on \<sigma>' (the \<circ> m) (dom m)) C"
        ...
        also have "... = Bv (\<not> (bval_of (signal_of (\<sigma>' A) \<tau>'' A 0) \<and> bval_of (signal_of (\<sigma>' B) \<tau>'' B 0)))"
          using sA sB by auto
        finally have IR0: "bval_of (signal_of (def C) (get_beh res) C 1) \<longleftrightarrow> \<not> (bval_of (signal_of (\<sigma>' A) \<tau>'' A 0) \<and> bval_of (signal_of (\<sigma>' B) \<tau>'' B 0))"
          by auto
        ...
        qed
          ...
          by auto
        then have ?thesis
          using IR IR0 hel le_less_linear
          using \<open>next_time 0 \<tau>'' = 1\<close> \<open>next_time 0 \<tau>'' \<le> Suc i\<close> le_Suc_eq by force }
      moreover
      { assume "bval_of (\<sigma>' C) \<longleftrightarrow> \<not> (bval_of (\<sigma>' A) \<and> bval_of (\<sigma>' B))"
       ...
       (\<not> (bval_of (signal_of (\<sigma>'' A) (\<tau>''(next_time 0 \<tau>'' := 0)) A i) \<and> bval_of (signal_of (\<sigma>'' B) (\<tau>''(next_time 0 \<tau>'' := 0)) B i)))"
        ...
        ultimately have **: "next_time 0 \<tau>'' \<le> i \<Longrightarrow>
           bval_of (signal_of (def C) (get_beh res) C (Suc i)) = (\<not> (bval_of (signal_of (\<sigma>' A) \<tau>'' A i) \<and> bval_of (signal_of (\<sigma>' B) \<tau>'' B i)))"
          by auto
        have "signal_of (\<sigma>' A) \<tau>'' A i = signal_of (\<sigma>' A) (?\<tau>'(0:=0)) A i"
          unfolding \<tau>''_def   using signal_of_trans_post by fastforce
        also have "... = signal_of (\<sigma>' A) ?\<tau>' A i"
        ...
        { assume "i < next_time 0 \<tau>''"
          ...
          { assume "Suc i = next_time 0 \<tau>''"
            ...
            hence "signal_of (def C) (get_beh res) C (Suc i) = \<sigma>'' C"
            ...
            with sA sB hel hel2 have ?thesis
              using `bval_of (\<sigma>' C) \<longleftrightarrow> \<not> (bval_of (\<sigma>' A) \<and> bval_of (\<sigma>' B))` by auto }
          moreover
          { assume "Suc i < next_time 0 \<tau>''"
            ...
            have "signal_of (def C) (get_beh res) C (Suc i) = signal_of (def C) (get_beh res) C 0"
            ...
            qed auto
            also have "... = \<sigma>' C"
              by (metis (no_types, lifting) \<open>0 \<le> Suc i\<close> \<open>\<And>t. t \<le> Suc i \<Longrightarrow> \<theta>'' t = get_beh res t\<close> \<open>\<theta>'' =
              0(0 := Some \<circ> \<sigma>')\<close> fun_upd_same trans_some_signal_of)
            finally have "signal_of (def C) (get_beh res) C (Suc i) = \<sigma>' C"
              by auto
            with sA sB hel hel2 have ?thesis
              using `bval_of (\<sigma>' C) \<longleftrightarrow> \<not> (bval_of (\<sigma>' A) \<and> bval_of (\<sigma>' B))` by auto }
          ultimately have "bval_of (signal_of (def C) (get_beh res) C (Suc i)) \<longleftrightarrow> (\<not> (bval_of (signal_of (def A) \<tau> A i) \<and> bval_of (signal_of (def B) \<tau> B i)))"
            by auto }
        hence "i < next_time 0 \<tau>'' \<Longrightarrow> bval_of (signal_of (def C) (get_beh res) C (Suc i)) \<longleftrightarrow>
                                   (\<not> (bval_of (signal_of (def A) \<tau> A i) \<and> bval_of (signal_of (def B) \<tau> B i)))"
          by auto
        with ** have ?thesis
          using hel hel2  using le_less_linear by auto }
      ultimately show ?thesis by auto
    next
      case none
