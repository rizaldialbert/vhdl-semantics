...
theory NAND_Hoare_Vector_Typed
...
definition nand_inv :: "sig assn2" where
  "nand_inv \<equiv> (\<lambda>tw. 
             lof_wline tw C (fst tw) = map2 (\<lambda>x y. x \<longrightarrow> \<not> y) (lof_wline tw A (fst tw - 1)) (lof_wline tw B (fst tw - 1)))"

definition nand_inv2 :: "sig assn2" where
  "nand_inv2 \<equiv> (\<lambda>tw. disjnt {A, B} (event_of tw) \<longrightarrow> (\<forall>i>fst tw. lof_wline tw C i = lof_wline tw C (fst tw)))"
...
proof -
  have bexpA: "bexp_wt \<Gamma> (Bsig A) (Lty ki len)" and bexpB: "bexp_wt \<Gamma> (Bsig B) (Lty ki len)" 
    using vector_type_nand3_axioms unfolding vector_type_nand3_def by (metis bexp_wt.intros(3))+
  ...
  also have "... = map2 (\<lambda>x y. x \<longrightarrow> \<not> y) (lof_wline tw A (fst tw)) (lof_wline tw B (fst tw))"
    using eval_world_raw_lv[OF bexpA `wityping \<Gamma> (snd tw)`]  eval_world_raw_lv[OF bexpB `wityping \<Gamma> (snd tw)`] 
...
proof -
  ...
  also have "... = map2 (\<lambda>x y. x \<longrightarrow> \<not> y) (lof_wline tw A (fst tw - 1)) (lof_wline tw B (fst tw - 1))"
...
lemma conc_wt_nand3:
  ...
  vector_type_nand3_axioms vector_type_nand3_def seq_wt.intros(4))
