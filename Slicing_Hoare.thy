...
lemma potential_tyenv:
  ...
  shows "\<exists>len>3. \<Gamma> IN = Lty len \<and> \<Gamma> OUT = Lty 2"
proof (rule seq_wt_cases(4)[OF assms])
  assume "bexp_wt \<Gamma> (Bslice IN 3 2) (\<Gamma> OUT)"
  then obtain len where "bexp_wt \<Gamma> (Bsig IN) (Lty len) \<and> 3 < len \<and> \<Gamma> OUT = Lty 2"
    by (metis add_2_eq_Suc add_Suc add_Suc_right add_diff_cancel_right' bexp_wt_cases(8)
    numeral_2_eq_2 numeral_3_eq_3)
  hence "bexp_wt \<Gamma> (Bsig IN) (Lty len)" and "3 < len" and "\<Gamma> OUT = Lty 2"
    by auto
  hence "\<Gamma> IN = Lty len"
    using bexp_wt_cases(9)[OF \<open>bexp_wt \<Gamma> (Bsig IN) (Lty len)\<close>] by metis
  thus ?thesis
    using \<open>3 < len\<close> \<open>\<Gamma> OUT = Lty 2\<close> by blast
qed
...
text \<open>Note that we need the assumption that @{term "type_of x = Lty 2"} as there is no
guarantee that @{term "beval_world_raw2 tw (Bslice IN 3 2) x"} would result in a list 
of length 2.\<close>

lemma inv_next_time:
  assumes "inv tw"
  assumes "beval_world_raw2 tw (Bslice IN 3 2) x" and "type_of x = Lty 2"
  ...
  proof (rule, rule)
    ...
    { assume "fst tw \<le> i \<and> i < next_time_world tw' - 1"
      ...
      proof -
        ...
        hence "state_of_world (snd tw) (fst tw) IN = Lv ?instw"
          unfolding state_of_world_def by simp
        show "lval_of x = nths ?instw {?idxtw - 3 .. ?idxtw - 2}"
          apply (rule beval_world_raw_cases[OF assms2], erule beval_cases)
          using \<open>state_of_world (snd tw) (fst tw) IN = Lv ?instw\<close> plus_1_eq_Suc by auto
      qed
      also have "... = nths ?ins' {?idx' - 3 .. ?idx' - 2}"
...
proof - 
  ...
  have "type_of (state_of_world (snd tw) (get_time tw) IN) = Lty len"
  ...
  show ?thesis
    ...
    using \<open>3 < len\<close> ** ***  by force
qed
...
lemma slicer_seq_hoare_next_time0:
  ...
  apply (rule Assign2_altI)
  using inv_next_time type_correctness_length[OF assms] inv_def gr_implies_not0 by blast
...
proof (rule, rule, rule)
  ...
  proof -
    ...
    hence "state_of_world (snd tw) (fst tw) IN = Lv ?ins"
      unfolding \<open>?ins = lof_wline tw IN (fst tw)\<close>state_of_world_def by simp
    have "lval_of x = nths ?ins {?idx - 3 .. ?idx - 2}"
      apply (rule beval_world_raw_cases[OF assms2])  
      apply (erule beval_cases)+
      using \<open>state_of_world (snd tw) (fst tw) IN = Lv ?ins\<close> 
      by (metis \<open>lof_wline (next_time_world tw', snd tw') IN (next_time_world tw') = lof_wline tw IN
      (get_time tw)\<close> diff_Suc_eq_diff_pred eval_nat_numeral(3) val.sel(2))
