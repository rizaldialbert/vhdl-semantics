...
theory Shift_Right_Hoare_Pos_Typed
...
lemma well_typed:
  ...
  by (rule seq_wt.intros(4))
     (metis (full_types) bexp_wt.intros(23) bexp_wt.intros(3) unsigned_shift_right_axioms
     unsigned_shift_right_def)

abbreviation "lof_wline tw sig n \<equiv> lval_of (wline_of tw sig n)"

definition inv :: "sig assn2" where
  "inv tw \<equiv> (lof_wline tw OUT (fst tw) = take len (replicate amount False @ lof_wline tw IN (fst tw - 1)))"
...
lemma inv_next_time:
  fixes tw
  defines "v \<equiv> eval_world_raw2 tw (Bshiftr (Bsig IN) amount)"
...
proof - 
  have bexpIN: "bexp_wt \<Gamma> (Bsig IN) (Lty Uns len)" 
    using unsigned_shift_right_axioms unfolding unsigned_shift_right_def by (metis bexp_wt.intros(3))+
  ...
  also have "... = take len (replicate amount False   @ lof_wline tw IN (fst tw))"
    using evalIN `length bsIN = len` unfolding v_def eval_world_raw.simps eval_arith.simps Let_def by auto
  finally show ?thesis
...
proof -
  ...
  also have "... = take len (replicate amount False @ lval_of (wline_of tw IN (get_time tw - 1)))"
    using `inv tw` unfolding inv_def by auto
  also have "... = take len (replicate amount False @ lval_of (wline_of tw IN (get_time tw)))"
...
lemma conc_stmt_wf:
  "conc_stmt_wf (shiftr amount)"
  unfolding shiftr_def conc_stmt_wf_def by auto  

lemma nonneg_delay_conc:
  "nonneg_delay_conc (shiftr amount)"
  unfolding shiftr_def by auto

lemma nonneg_delay_conc':
  "nonneg_delay_conc ( process {IN} : Bassign_trans OUT (Bshiftr (Bsig IN) amount) 1)"
  using nonneg_delay_conc unfolding shiftr_def by auto

lemma conc_wt:
  "conc_wt \<Gamma> (shiftr amount)"
  unfolding shiftr_def  by (meson conc_wt.intros(1) well_typed)

lemma conc_wt':
  "conc_wt \<Gamma> ( process {IN} : Bassign_trans OUT (Bshiftr (Bsig IN) amount) 1)"
  using conc_wt unfolding shiftr_def by auto

lemma conc_sim2':
  "\<Gamma> \<turnstile>\<^sub>s \<lbrace>\<lambda>tw. inv tw \<and> inv2 tw\<rbrace> shiftr amount \<lbrace>\<lambda>tw. inv tw \<and> inv2 tw\<rbrace>"
  apply (rule While_Suc)
  apply (rule Conseq'[where P="wp3_conc \<Gamma> (shiftr amount) (\<lambda>tw. inv  (fst tw + 1, snd tw) \<and> 
                                                              inv2 (fst tw + 1, snd tw))", rotated])
  apply (rule wp3_conc_is_pre, rule conc_stmt_wf, rule nonneg_delay_conc, rule conc_wt, simp)
  unfolding shiftr_def wp3_conc_single'[OF conc_wt' nonneg_delay_conc'] wp3_fun.simps
  using inv_next_time inv2_next_time conc_hoare conc_hoare2 by presburger

lemma nonneg_delay_add:
  " nonneg_delay (Bassign_trans OUT (Bshiftr (Bsig IN) amount) 1)"
  using nonneg_delay_conc' by auto

lemma init_sat_nand_inv_comb:
  "init_sim2_hoare_wt \<Gamma> (\<lambda>tw. fst tw = 0) (shiftr amount) (\<lambda>tw. inv tw \<and> inv2 tw)"
...
lemma correctness:
  assumes "sim_fin2 w (i + 1) (shiftr amount) tw'" and "wityping \<Gamma> w"
  shows "lof_wline tw' OUT (i + 1) = take len (replicate amount False @ lof_wline tw' IN (i))"
  using grand_correctness[OF assms conc_stmt_wf conc_wt nonneg_delay_conc conc_sim2' init_sat_nand_inv_comb]
  unfolding shiftr_def inv_def  by (metis (no_types, lifting) add_diff_cancel_right' assms(1)
