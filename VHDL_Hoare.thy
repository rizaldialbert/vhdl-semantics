...
lemma purge'_trans_post_preserve_non_stuttering_lv: 
  ...
  assumes "\<forall>n. n \<le> t \<longrightarrow>  \<tau> n = 0"
  shows "non_stuttering (to_trans_raw_sig \<tau>'') \<sigma> sig"
  using purge'_trans_post_preserve_non_stuttering_lv_strict 
  by (simp add: purge'_trans_post_preserve_non_stuttering_lv_strict \<tau>''_def \<tau>'_def assms(1) assms(4))
...
lemma post_raw_preserves_non_stuttering:
  ...
  shows   "non_stuttering (to_trans_raw_sig \<tau>') \<sigma> sig"
  using post_raw_preserves_non_stuttering_strict assms 
  by (simp add: post_raw_preserves_non_stuttering_strict)
...
lemma trans_post_preserves_non_stuttering:
  ...
  using assms trans_post_preserves_non_stuttering_strict 
  by (metis order.order_iff_strict)
...
proof (rule)+
  ...
  proof -
    have "\<forall>s. non_stuttering (to_trans_raw_sig \<tau>'') \<sigma> s"
      ...
      by (simp add: context_invariant_def)
    thus ?thesis
...
lemma beval_beval_world_raw_ci:
  ...
  shows "beval_raw t \<sigma> \<gamma> \<theta> def exp = beval_world_raw w t exp"
proof -
  have 0: "\<And>n. n < t \<Longrightarrow> \<tau> n = 0" and
       1: "\<gamma> = {s. \<sigma> s \<noteq> signal_of (def s) \<theta> s t}" and
       3: "\<And>n. t < n \<Longrightarrow> \<theta> n = 0"
